...
# defines a function to predict the waste type from text description using the ML model or fallback 
def predict_from_text(description, model=None, vectorizer=None, encoder=None):
    ...
    # uses the ML model if available
    if model is not None and vectorizer is not None and encoder is not None:
        try:
            ...
            # gets the predicted category
            category = encoder.inverse_transform([prediction])[0]

        # maps the category to a user frienfly format with emojis 
            category_mapping = {
            ...
            # falls back to rule-based prediction in case of an error 
            return rule_based_prediction(description)
    else:
        # falls back to rule-based prediction if the model is not available
        logger.info("ML model not available, using rule-based prediction")
        return rule_based_prediction(description)

# defines a function to predict the waste type from an image using the ML model or fallback
def predict_from_image(img, model=None, class_names=None):
    """Predict waste type from image with fallback to color-based"""
    if model is None or class_names is None:
        # falls back to color-based prediction if the model is not available
        logger.info("Image model not available, using color-based prediction")
        return simple_image_prediction(img)
        
    try:
        # ensures Tensorflow is available to run the model 
        import tensorflow as tf
        from tensorflow.keras.preprocessing import image as keras_image
        
        # reprocesses the image to the required size and format 
        img = img.resize((224, 224))
        img_array = keras_image.img_to_array(img)
        img_array = np.expand_dims(img_array, axis=0)
        img_array = img_array / 255.0
        
        # actually makes the prediction
        predictions = model.predict(img_array)
        class_idx = np.argmax(predictions[0])
        confidence = float(np.max(predictions[0]))
        
        # gets the predicted category with the class index
